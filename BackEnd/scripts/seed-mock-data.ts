...
async function seedMockData() {
  try {
    console.log('🌱 SEED MOCK DATA VÀO DATABASE');
    console.log('================================\n');
    ...
    // 3. Seed Departments từ dev-server.ts
    console.log('🏢 Seeding Departments...');
    const departments = [
      { name: 'Customer Support', description: 'General customer support', isActive: true },
      { name: 'Technical Support', description: 'Technical assistance', isActive: true },
      { name: 'Sales', description: 'Sales inquiries', isActive: true },
    ...
    for (const dept of departments) {
      try {
        await sql.query`
          ...
          VALUES (${dept.name}, ${dept.description}, ${dept.isActive ? 1 : 0}, GETDATE())
        `;
    ...
    // 4. Seed Agents từ dev-server.ts
    console.log('👨‍💼 Seeding Agents...');
    const agents = [
      { name: 'Alice Johnson', email: 'alice@muji.com', departmentId: 1, isActive: true },
      { name: 'Bob Smith', email: 'bob@muji.com', departmentId: 2, isActive: true },
      { name: 'Carol Davis', email: 'carol@muji.com', departmentId: 3, isActive: true },
      { name: 'David Wilson', email: 'david@muji.com', departmentId: 4, isActive: true }
    ];

    for (const agent of agents) {
      try {
        await sql.query`
          INSERT INTO Agents (AgentName, Email, DepartmentID, IsActive, CreatedAt)
          VALUES (${agent.name}, ${agent.email}, ${agent.departmentId}, ${agent.isActive ? 1 : 0}, GETDATE())
        `;
        console.log(`✅ Agent created: ${agent.name}`);
      } catch (error) {
        console.log(`⚠️ Agent ${agent.name} might already exist`);
      }
    }
    console.log('');

    // 5. Seed Sample Tickets từ dev-server.ts
    ...
    const tickets = [
      {
        ...
        description: 'I have a question about product availability',
        priority: 'Medium',
        status: 'Open'
      },
      {
      ...
      },
      {
        ...
        description: 'When will my order be shipped?',
        priority: 'Low',
        status: 'Resolved'
      }
    ];
    ...
    for (const ticket of tickets) {
      try {
        await sql.query`
          INSERT INTO Tickets (CustomerID, DepartmentID, AgentID, Subject, Description, Priority, Status, CreatedAt)
          VALUES (${ticket.customerId}, ${ticket.departmentId}, ${ticket.agentId}, ${ticket.subject}, ${ticket.description}, ${ticket.priority}, ${ticket.status}, GETDATE())
        `;
    ...
    // 6. Seed Sample Orders từ dev-server.ts
    ...
    console.log('   ✅ Departments: 4 items');
    console.log('   ✅ Agents: 4 items');
    console.log('   ✅ Tickets: 3 items');
