...
export class TicketController {
  static async createTicket(req: any, res: Response) {
    try {
      ...
      // Convert string IDs to numbers for database compatibility
      const ticketData = {
        ...validatedData,
        departmentId: validatedData.departmentId ? parseInt(validatedData.departmentId) : undefined
      };
      const ticket = await TicketService.createTicket(parseInt(req.user.userId), ticketData);
  ...
  static async getTicketById(req: any, res: Response) {
    try {
      const { id } = req.params;
      const ticket = await TicketService.getTicketById(parseInt(id), req.user.role, parseInt(req.user.userId));
  ...
  static async updateTicket(req: any, res: Response) {
    try {
      ...
      const updateData = {
        ...validatedData,
        departmentId: validatedData.departmentId ? parseInt(validatedData.departmentId) : undefined,
        assigneeId: validatedData.assigneeId ? parseInt(validatedData.assigneeId) : undefined
      };
      
      const ticket = await TicketService.updateTicket(parseInt(id), updateData, parseInt(req.user.userId));
