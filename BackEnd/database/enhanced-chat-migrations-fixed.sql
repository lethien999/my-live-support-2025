-- Enhanced Chat System Database Migrations - Fixed for current schema
...
-- 2. Add missing columns to ChatRooms table
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('ChatRooms') AND name = 'LastMessage')
BEGIN
    ALTER TABLE ChatRooms ADD LastMessage NVARCHAR(MAX) NULL;
END

IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('ChatRooms') AND name = 'LastMessageAt')
BEGIN
    ALTER TABLE ChatRooms ADD LastMessageAt DATETIME2 NULL;
END

-- 3. Create FileStorage table for file uploads
...
-- 4. Create ChatNotifications table for real-time notifications
...
-- 5. Create UserSessions table for tracking online status
...
-- 6. Create TypingStatus table for typing indicators
...
-- Procedure to get online users in a room (Fixed for current schema)
...
-- 9. Create triggers for automatic operations

-- Trigger to update room last message when new message is inserted (Fixed for current schema)
